...
def arg_parse(is_train=False):
    ...
    ## dataloader
    parser.add_argument('--dataset_name', type=str, default='interx', help='dataset directory')
    parser.add_argument('--motion_rep', type=str, default='smpl', help='how is the motion represented')
    parser.add_argument('--data_rep', type=str, default='rot6d', help='preprocessed motion representation to load')
    parser.add_argument('--use_processed_loader', action="store_true", help='use preprocessed motion dataloader')
    parser.add_argument('--fast_mode', action="store_true", help='skip kinematics-heavy losses for faster training')
    ...
    ## unified vq + captioning
    parser.add_argument('--caption_loss_weight', type=float, default=2.0, help='weight for captioning loss')
    parser.add_argument('--caption_num_layers', type=int, default=1, help='caption decoder GRU layers')
    parser.add_argument('--uni_transformer_layers', type=int, default=2, help='latent transformer layers per branch')
    parser.add_argument('--uni_transformer_heads', type=int, default=8, help='latent transformer attention heads')
    parser.add_argument('--uni_transformer_ff_size', type=int, default=2048, help='latent transformer FFN hidden size')
    parser.add_argument('--uni_dropout', type=float, default=0.1, help='dropout used in unified latent/text branches')
    parser.add_argument(
        '--init_vq_ckpt',
        type=str,
        default='checkpoints/interx/vq_rot6d/model/finest.tar',
        help='pretrained vanilla VQ checkpoint used to initialize and freeze the unified VQ encoder',
    )
