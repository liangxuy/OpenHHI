...
class RVQVAE(nn.Module):
    def __init__(self,
        ...
        self.data_rep = str(getattr(args, 'data_rep', 'rot6d')).lower()
        ...
        self._decoder_invalid_warn_count = 0
    ...
    def preprocess(self, x):
        ...
        elif self.dataset_name == "interx" and self.data_rep == "noncanonical":
            if x.ndim != 3 or x.shape[-1] != NONCANONICAL_PERSON_DIM:
                raise ValueError(
                    f"noncanonical expects [B, T, {NONCANONICAL_PERSON_DIM}] input, got {tuple(x.shape)}"
                )
            pos = x[..., :NONCANONICAL_POS_DIM].reshape(x.shape[0], x.shape[1], NONCANONICAL_JOINTS, 3)
            vel = x[..., NONCANONICAL_POS_DIM:NONCANONICAL_POS_DIM + NONCANONICAL_VEL_DIM].reshape(
                x.shape[0], x.shape[1], NONCANONICAL_JOINTS, 3
            )
            rot = x[..., NONCANONICAL_POS_DIM + NONCANONICAL_VEL_DIM:
                     NONCANONICAL_POS_DIM + NONCANONICAL_VEL_DIM + NONCANONICAL_ROT_DIM].reshape(
                x.shape[0], x.shape[1], NONCANONICAL_JOINTS, 6
            )
            fc = x[..., -NONCANONICAL_FC_DIM:]
            body = torch.cat([pos, vel, rot], dim=-1)  # [B, T, 55, 12]
            aux_joint = torch.zeros(x.shape[0], x.shape[1], 1, 12, device=x.device, dtype=x.dtype)
            aux_joint[..., :NONCANONICAL_FC_DIM] = fc.unsqueeze(2)
            joints = torch.cat([body, aux_joint], dim=2)  # [B, T, 56, 12]
        else:
            joints = x
    ...
    def postprocess(self, x):
        ...
        if self.dataset_name == "interhuman":
            ...
            if x.ndim != 4 or x.shape[2] < NONCANONICAL_JOINTS + 1 or x.shape[3] < 12:
                raise ValueError(f"Unexpected decoded tensor shape for noncanonical: {tuple(x.shape)}")
            body = x[:, :, :NONCANONICAL_JOINTS, :12]
            pos = body[..., :3].reshape(x.shape[0], x.shape[1], -1)
            vel = body[..., 3:6].reshape(x.shape[0], x.shape[1], -1)
            rot = body[..., 6:12].reshape(x.shape[0], x.shape[1], -1)
            fc = x[:, :, NONCANONICAL_JOINTS, :NONCANONICAL_FC_DIM]
            x = torch.cat([pos, vel, rot, fc], dim=-1)
        return x
    ...
    def forward_decoder(self, x, soft_lookup=False):
        if not soft_lookup:
            ...
            if invalid_mask.any():
                ...
                if self._decoder_invalid_warn_count < 3:
                    x_det = x.detach()
                    min_idx = int(x_det.min().item()) if x_det.numel() else 0
                    max_idx = int(x_det.max().item()) if x_det.numel() else 0
                    print(
                        f"[Warning] RVQVAE.forward_decoder got {invalid_count} invalid code indices "
                        f"(range [{min_idx}, {max_idx}], valid [-1, {self.num_code - 1}]); replacing with -1."
                    )
                self._decoder_invalid_warn_count += 1
                x = x.clone()
                x[invalid_mask] = -1
