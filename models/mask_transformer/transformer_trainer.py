...
from eval_uni import evaluation_during_training
...
class MaskTransformerTrainer:
    def __init__(self, args, t2m_transformer, vq_model):
        ...
        self.vq_dim_joint = int(getattr(args, 'vq_dim_joint', getattr(args, 'dim_joint', 6)))
        self.vq_model.eval()
    ...
    def forward(self, batch_data):
        ...
        if self.opt.dataset_name == "interhuman":
            ...
            if motions.shape[-1] % 2 != 0:
                raise ValueError(f"Expected even motion feature dim for two-person split, got {motions.shape}")
            expected_dim = self.vq_dim_joint * 2
            if motions.shape[-1] != expected_dim:
                raise ValueError(
                    f"Motion feature dim mismatch: got {motions.shape[-1]}, expected {expected_dim} "
                    f"(2 * vq_dim_joint={self.vq_dim_joint})."
                )
            motion1, motion2 = motions.split(self.vq_dim_joint, dim=-1)
