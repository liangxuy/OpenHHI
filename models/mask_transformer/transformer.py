...
class EmbedAction(nn.Module):
    def __init__(self, num_actions, latent_dim):
        ...
        self.latent_dim = latent_dim
        self.weight = nn.Parameter(torch.empty(num_actions, latent_dim))
    ...
    def forward(self, action_ids):
        ...
        if action_ids.ndim > 1:
            action_ids = action_ids.reshape(action_ids.shape[0], -1)[:, 0]
        action_ids = action_ids.to(torch.long)
        return self.weight[action_ids] + self.bias

    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
                              missing_keys, unexpected_keys, error_msgs):
        weight_key = prefix + 'weight'
        legacy_shape = (self.latent_dim, self.num_actions)
        if weight_key in state_dict and tuple(state_dict[weight_key].shape) == legacy_shape:
            state_dict[weight_key] = state_dict[weight_key].transpose(0, 1).contiguous()
...
class MaskTransformer(nn.Module):
    def __init__(self, code_dim, cond_mode, latent_dim=256, ff_size=1024, num_layers=8,
        ...
        self.encode_action = self.prepare_action_ids
        self.enc_action = self.encode_action

        # if self.cond_mode != 'no_cond':
        if self.cond_mode == 'text':
            self.cond_emb = nn.Linear(self.clip_dim, self.latent_dim)
        elif self.cond_mode == 'action':
            self.cond_emb = EmbedAction(self.num_actions, self.latent_dim)
    ...
    def prepare_action_ids(self, action_ids):
        if torch.is_tensor(action_ids):
            encoded = action_ids
        else:
            encoded = torch.as_tensor(action_ids)

        if encoded.ndim == 0:
            encoded = encoded.unsqueeze(0)
        if encoded.ndim > 1:
            encoded = encoded.reshape(encoded.shape[0], -1)[:, 0]
        return encoded.to(torch.long)

    def encode_condition(self, cond, force_mask=False):
        if self.cond_mode == 'text':
            return self.cond_emb(self.mask_cond(cond, force_mask=force_mask))
        if self.cond_mode == 'action':
            return self.mask_cond(self.cond_emb(cond), force_mask=force_mask)
    ...
    def trans_forward(self, motion_ids, cond, padding_mask, force_mask=False):
        ...
        :param cond: (b, embed_dim) for text, (b,) or (b, 1) for action
        ...
        cond = self.encode_condition(cond, force_mask=force_mask)
    ...
    def forward(self, ids, y, m_lens):
        ...
        if self.cond_mode == 'text':
            ...
            cond_vector = self.enc_action(y).to(device)
        elif self.cond_mode == 'uncond':
    ...
    def forward_reaction(self, active_ids, reactive_ids, y, m_lens):
        '''
        :param active_ids: (b, n) active-person token ids
        :param reactive_ids: (b, n) passive-person token ids
        :param y: raw text for cond_mode=text, (b,) for cond_mode=action
        :param m_lens: (b,) latent lengths before spatial-token repetition
        :return:
        '''
        bs, ntokens = reactive_ids.shape
        device = reactive_ids.device

        max_len = ntokens // self.nbp
        full_non_pad_mask = lengths_to_mask(m_lens, max_len)
        full_non_pad_mask = full_non_pad_mask.repeat(1, self.nbp)
        active_non_pad_mask = full_non_pad_mask[:, :ntokens]
        reactive_non_pad_mask = full_non_pad_mask[:, ntokens:]

        active_ids = torch.where(active_non_pad_mask, active_ids, self.pad_id)
        reactive_ids = torch.where(reactive_non_pad_mask, reactive_ids, self.pad_id)
        ...
        batch_randperm = torch.rand((bs, ntokens), device=device).argsort(dim=-1)
        mask = batch_randperm < num_token_masked.unsqueeze(-1)
        mask &= reactive_non_pad_mask

        labels = torch.where(mask, reactive_ids, self.mask_id)

        x_reactive_ids = reactive_ids.clone()
        mask_rid = get_mask_subset_prob(mask, 0.1)
        rand_id = torch.randint_like(x_reactive_ids, high=self.opt.num_tokens)
        x_reactive_ids = torch.where(mask_rid, rand_id, x_reactive_ids)
        mask_mid = get_mask_subset_prob(mask & ~mask_rid, 0.88)
        x_reactive_ids = torch.where(mask_mid, self.mask_id, x_reactive_ids)

        input_ids = torch.cat([active_ids, x_reactive_ids], dim=-1)
        logits = self.trans_forward(input_ids, cond_vector, ~full_non_pad_mask, force_mask)
        reactive_logits = logits[:, :, ntokens:]
        ce_loss, pred_id, acc = cal_performance(reactive_logits, labels, ignore_index=self.mask_id)

        reactive_logits = reactive_logits.permute(0, 2, 1)
        if self.opt.step_unroll:
            su_ce_loss, su_pred_id, su_acc = self.step_unroll_forward_reaction(
                active_ids,
                x_reactive_ids,
                mask_mid,
                labels,
            ...
            return ce_loss + (self.opt.step_unroll * su_ce_loss), (acc + self.opt.step_unroll * su_acc) / 2, pred_id, su_pred_id, reactive_logits

        return ce_loss, acc, pred_id, None, reactive_logits
    ...
    def step_unroll_forward_reaction(self, active_ids, prev_masked_reactive_ids, prev_mask, prev_labels, logits, cond_vector, full_non_pad_mask, force_mask):
        total_timesteps = 20
        prev_rand_mask_probs = prev_mask.count_nonzero(dim=-1).float() / prev_mask.shape[-1]
        prev_rand_time = self.noise_schedule_backward(prev_rand_mask_probs)
        ...
        retained_preds = torch.logical_and(prev_mask, ~mask)
        labels = torch.where(retained_preds, self.mask_id, prev_labels)
        reactive_ids = torch.where(retained_preds, pred_ids, prev_masked_reactive_ids)

        step_unroll_logits = self.trans_forward(
            torch.cat([active_ids, reactive_ids], dim=-1),
            cond_vector,
            ~full_non_pad_mask,
            force_mask,
        )
        step_unroll_logits = step_unroll_logits[:, :, active_ids.shape[1]:]
        return cal_performance(step_unroll_logits, labels, ignore_index=self.mask_id)
    ...
    def generate_reaction(self,
        ...
        device = next(self.parameters()).device
        if motion1_ids.shape[1] % self.nbp != 0:
            raise ValueError(
                f"Active motion token length {motion1_ids.shape[1]} is not divisible by nbp={self.nbp}."
            )

        seq_len = motion1_ids.shape[1] // self.nbp
        batch_size = len(m_lens)
        m_lens = m_lens.clamp(min=1, max=seq_len)
