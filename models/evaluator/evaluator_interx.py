...
from data.interx_utils import InterxNormalizerTorch, InterxRot6dEvaluator
from utils.interx_text import resolve_interx_glove_dir
...
NONCANONICAL_TOTAL_DIM = NONCANONICAL_PERSON_DIM * 2
    ...
    def __getitem__(self, item):
        ...
        for mm_motion in mm_motions:
            ...
            if len(motion) < self.max_motion_length:
                if motion.ndim == 3:
                    pad = np.zeros((self.max_motion_length - len(motion), motion.shape[1], motion.shape[2]), dtype=motion.dtype)
                elif motion.ndim == 2:
                    pad = np.zeros((self.max_motion_length - len(motion), motion.shape[1]), dtype=motion.dtype)
                else:
                    raise ValueError(f"Unsupported motion rank {motion.ndim}")
                motion = np.concatenate([motion, pad], axis=0)
            motion = motion[None, :]
...
def get_motion_loader(batch_size, model, trans, ground_truth_dataset, device, mm_num_samples, mm_num_repeats, file, opt, time_steps, cond_scale, topkr):
    ...
    start = time.time()
    _, glove_dir = resolve_interx_glove_dir(opt.data_root, getattr(opt, 'text_source', None), require_exists=True)
...
def get_dataset_motion_loader(opt, batch_size):
    ...
    if opt.dataset_name == 'interx':
        ...
        dataset_kwargs = {}
        DatasetClass = Text2MotionDatasetV2HHI
        if getattr(opt, 'use_processed_loader', False):
            DatasetClass = Text2MotionDatasetV2HHIProcessed
            dataset_kwargs['data_rep'] = getattr(opt, 'data_rep', 'rot6d')

        _, glove_dir = resolve_interx_glove_dir(opt.data_root, getattr(opt, 'text_source', None), require_exists=True)
        dataset = DatasetClass(opt, 
                               pjoin(opt.data_root, 'splits/test.txt'), 
                               WordVectorizer(glove_dir, 'interx_vab'), 
...
def build_models(opt):
    ...
    text_mot_match_name = getattr(opt, 'text_mot_match_name', 'text_mot_match')
    checkpoint_path = pjoin(opt.checkpoints_dir, opt.dataset_name, text_mot_match_name, 'model', 'finest.tar')
    if not os.path.isfile(checkpoint_path):
        raise FileNotFoundError(f'Inter-X evaluation checkpoint not found: {checkpoint_path}')
    print(f'Loading Evaluation Model Wrapper from {checkpoint_path}')
...
class EvaluatorModelWrapper(object):
    ...
    def __init__(self, opt):
        self.data_rep = str(getattr(opt, 'data_rep', 'rot6d')).lower()
        if self.data_rep not in InterxRot6dEvaluator.SUPPORTED_REPRESENTATIONS:
            raise ValueError(f"Unsupported data representation {self.data_rep}. Choose from {InterxRot6dEvaluator.SUPPORTED_REPRESENTATIONS}")
        self.eval_converter = InterxRot6dEvaluator(data_rep=self.data_rep)
        preserved_max_text_len = getattr(opt, 'max_text_len', None)

        # The retrieval evaluator checkpoint is trained on concatenated
        # two-person rot6d+root(6) layout => 56 * 12 per frame.
        opt.dim_pose = 56 * 12
        ...
        if opt.dataset_name == 'hhi':
            ...
            opt.max_text_len = 35
        if preserved_max_text_len is not None:
            opt.max_text_len = int(preserved_max_text_len)
    ...
    def _noncanonical_flat_to_rotroot(self, motion_flat):
        """
        Convert one-person noncanonical flat features [B, T, 664]
        to evaluator-ready rot6d+root6 layout [B, T, 56, 6].
        """
        if motion_flat.ndim != 3 or motion_flat.shape[-1] != NONCANONICAL_PERSON_DIM:
            raise ValueError(
                f"Expected noncanonical one-person motion [B, T, {NONCANONICAL_PERSON_DIM}], got {motion_flat.shape}"
            )
        pos = motion_flat[..., :NONCANONICAL_POS_DIM].reshape(
            motion_flat.shape[0], motion_flat.shape[1], NONCANONICAL_JOINTS, 3
        )
        vel = motion_flat[..., NONCANONICAL_POS_DIM:NONCANONICAL_POS_DIM + NONCANONICAL_VEL_DIM].reshape(
            motion_flat.shape[0], motion_flat.shape[1], NONCANONICAL_JOINTS, 3
        )
        rot = motion_flat[..., NONCANONICAL_POS_DIM + NONCANONICAL_VEL_DIM:
                          NONCANONICAL_POS_DIM + NONCANONICAL_VEL_DIM + NONCANONICAL_ROT_DIM].reshape(
            motion_flat.shape[0], motion_flat.shape[1], NONCANONICAL_JOINTS, 6
        )
        root6 = torch.cat([pos[..., 0, :], vel[..., 0, :]], dim=-1)
        return torch.cat([rot, root6.unsqueeze(2)], dim=2)

    def _to_eval_layout(self, motions):
        """
        Convert incoming two-person motions to evaluator layout (rot6d + root6
        per person), concatenated on the last dimension.
        """
        if self.data_rep == 'noncanonical' and motions.ndim == 3:
            if motions.shape[-1] != NONCANONICAL_TOTAL_DIM:
                raise ValueError(
                    f"Expected noncanonical two-person motion dim {NONCANONICAL_TOTAL_DIM}, got {motions.shape}"
                )
            motion1, motion2 = motions.split(NONCANONICAL_PERSON_DIM, dim=-1)
            motion1 = self._noncanonical_flat_to_rotroot(motion1)
            motion2 = self._noncanonical_flat_to_rotroot(motion2)
        else:
            if motions.shape[-1] % 2 != 0:
                raise ValueError(f"Expected even last dim for two-person motion, got {motions.shape}")
            motion1, motion2 = motions.split(motions.shape[-1] // 2, dim=-1)
            motion1 = self.eval_converter.forward(motion1)
            motion2 = self.eval_converter.forward(motion2)
        motions_eval = torch.cat([motion1, motion2], dim=-1)
        if motions_eval.shape[-1] != 12:
            raise ValueError(f"Evaluator expects per-joint dim 12 after conversion, got {motions_eval.shape}")
        return motions_eval
    ...
    def prepare_motion(self, motions):
        ...
        motions = self._to_eval_layout(motions)
    ...
    def get_motion_embeddings(self, batch):
        ...
        if m_lens.ndim > 1:
            if m_lens.shape[0] != 1:
                raise ValueError(f"Expected a singleton loader batch dimension for m_lens, got shape {tuple(m_lens.shape)}")
            m_lens = m_lens[0]

        if motions.ndim >= 4 and motions.shape[0] == 1 and motions.shape[1] == m_lens.shape[0]:
            motions = motions[0]
        with torch.no_grad():
            if motions.shape[0] != m_lens.shape[0]:
                raise ValueError(f"Mismatched multimodality batch: motions batch {motions.shape[0]} vs m_lens batch {m_lens.shape[0]}")
